...
def json_obj_reader(file):
   ...
   for i in range(annotation_length):
    #   if len(fcc_data['annotation'][0]['objects_all_string'])==1:
    ...
    #    return verbs_in_szene
     if len(fcc_data['annotation'][0]['objects_all_string'])!=0:
      objects_in_szene.append(fcc_data['annotation'][i]['objects_all_string'])
     return objects_in_szene
...
def action_all_counter(folder):
   ...
   for i in range(file_number):
      ...
      current_scene_obj=json_obj_reader(file_path)
      for j in current_scene_obj:
