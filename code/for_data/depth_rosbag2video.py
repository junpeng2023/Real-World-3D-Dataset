...
class RosVideoWriter():
    ...
    def addBag(self, filename):
        ...
        for topic, msg, t in bag.read_messages(connection_filter=self.filter_image_msgs, start_time=self.opt_start, end_time=self.opt_end):
            ...
            #                 normalized_depth = cv2.normalize(cv_image, None, 0, 255, cv2.NORM_MINMAX, dtype=cv2.CV_8U)
            ...
            #             print('unsupported jpeg format:', msg.format, '.', topic)
             if isinstance(msg, CompressedImage):
                ...
                if "jpeg" in msg.format:
                    # Process JPEG compressed images
                    # ... your code for handling JPEG compressed images ...
                    if msg.format.find("8")!=-1 and (msg.format.find("rgb")!=-1 or msg.format.find("bgr")!=-1 or msg.format.find("bgra")!=-1 ):
                        if self.opt_display_images:
                            #np_arr = np.fromstring(msg.data, np.uint8)
                            #cv_image = cv2.imdecode(np_arr, cv2.CV_LOAD_IMAGE_COLOR)
                ...
                # Handle raw images
                pix_fmt = None
                if "mono8" in msg.encoding or "8UC1" in msg.encoding:
                    pix_fmt = "gray"
                elif "bgra" in msg.encoding:
                    pix_fmt = "bgra"
                # ... other encodings ...
                ...
                if pix_fmt:
                    ...
                    print("Unknown message type for topic:", topic)
